...
function openRoutineCreateModal() {
  ...
  // Cron schedule (shown only for cron)
  ...
  // Event fields (shown only for event)
  ...
  // Prompt (always shown, label changes based on action type)
  ...
  // Show/hide conditional trigger fields
  ...
  // Update prompt label based on action type
  ...
  saveBtn.addEventListener('click', function() {
    ...
    if (!prompt) {
      errMsg.textContent = (actionSel.value === 'full_job' ? 'Description' : 'Prompt') + ' is required.';
      errMsg.style.display = 'block';
      promptInput.focus();
      return;
    }
